...
module.exports = function (request, response, next) {
  ...
  if (!request.query.departureTime) {
    response.redirect(302, request.locals.config.baseUri + '/routes/' + request.params.routeid + '/?departureTime=' + encodeURIComponent((new Date()).toISOString().substr(0,10)));
  } else {
    ...
    if (!dateParam){
      ...
      connections.getRoute(request.params.routeid, dateParam, function (error, connectionsStream) {
        if (error) {
          next(error);
        } else {
          //Create output
