...
module.exports = function (request, response, next) {
  ...
  if (!request.query.date) {
    response.redirect(302, request.locals.config.baseUri + '/trips/?date=' + encodeURIComponent((new Date()).toISOString().substr(0,10)));
  } else {
    var dateParam = new Date(request.query.date);
    if (!dateParam){
      response.redirect(302, request.locals.config.baseUri + '/trips/?date=' + encodeURIComponent((new Date()).toISOString().substr(0,10)));
    } else {
...
//        "@id" : request.locals.page.getCurrentPage(),
        ...
        "search" : {
          "@type" : "IriTemplate",
          "template" : request.locals.config.baseUri + "/trips/{?date}",
          "variableRepresentation" : "BasicRepresentation",
          "mapping" : {
            "@type" : "IriTemplateMapping",
            "variable" : "departureTime",
            "required" : true,
            "property" : "http://semweb.mmlab.be/ns/linkedconnections#tripDateQuery"
          }
        }
      });
